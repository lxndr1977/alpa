<section class="max-w-7xl mx-auto py-20 px-6 lg:px-8">
   <div class="flex items-center justify-between mb-8">
      <h2 class="text-2xl font-medium text-neutral-900">Segmentos</h2>
   </div>

   <div class="relative">
      <div class="swiper swiper-segment relative overflow-visible">
         <div class="swiper-wrapper">
            @foreach ($segments as $segment)
               <div class="swiper-slide">
                  <a href="{{ url('segmentos/' . $segment->slug) }}"
                     class="block group border border-neutral-200 rounded-xl overflow-hidden bg-white hover:shadow-lg transition">

                     @php $media = $segment->getFirstMedia('banner') ?? $segment->getFirstMedia('image'); @endphp
                     ...
                     <div class="p-4 text-center">
                        <h3 class="text-lg font-semibold text-neutral-800">{{ $segment->name }}</h3>
                     </div>
                  </a>
               </div>
            @endforeach
         ...
         {{-- Botões de navegação --}}
         <button
            class="swiper-button-prev-segment absolute top-1/2 -translate-y-1/2 left-0 md:-left-10 z-10
         ...
         <button
            class="swiper-button-next-segment absolute top-1/2 -translate-y-1/2 right-0 md:-right-10 z-10
                  w-10 h-10 flex items-center justify-center rounded-full 
                  bg-white/90 border border-neutral-300 shadow-md
                  text-neutral-600 hover:bg-neutral-100 hover:scale-105 transition">
